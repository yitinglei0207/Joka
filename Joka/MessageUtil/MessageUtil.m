...
+ (NSDictionary *)reformedMessageToDic:(AnIMMessage *)message
{
    ...
    NSDictionary *dic = @{@"msgId":message.msgId,
                          ...
                          @"currentClientId":[JKLightspeedManager manager].clientId};
    return dic;
}
...
{
    if (![message.from isEqualToString:[JKLightspeedManager manager].clientId]) {
...
+ (NSInteger)getAllUnreadCount
{
    ...
    [CoreDataUtil getWithEntityName:@"HXMessage"
                          predicate:[NSPredicate predicateWithFormat
                                     ...
                                     ,0,[JKLightspeedManager manager].clientId,[JKLightspeedManager manager].clientId]];
...
{
    [[[JKLightspeedManager manager]anIM] getOfflineHistory:[JKLightspeedManager manager].clientId
                                             limit:30
...
{
    [[[JKLightspeedManager manager]anIM] getOfflineTopicHistory:[JKLightspeedManager manager].clientId
                                                  limit:30
                                                success:^(NSArray* messages ,int count){
...
+ (void)saveChatMessageIntoDB:(NSArray *)messages withTargetClientId:(NSString *)targetClientId
{
    for (AnIMMessage *message in messages)
    {
        ...
        HXChat *chatSession = [ChatUtil createChatSessionWithCurrentClientId:[JKLightspeedManager manager].clientId targetClientId:targetClientId currentUserName:@"" targetUserName:@""];
        ...
        if ([JKLightspeedManager manager].remoteNotificationInfo[@"from"]) {
            /* reset notification info */
            [JKLightspeedManager manager].remoteNotificationInfo = [[NSMutableDictionary alloc]initWithCapacity:0];
...
+ (void)saveTopicMessageIntoDB:(NSArray *)messages
{
    for (AnIMMessage *message in messages)
    {
        ...
        if (chatSession == nil) {
            ...
            [[[JKLightspeedManager manager]anIM] getTopicInfo:dic[@"topicId"]];
        }
        ...
        /* for click push notification */
        if ([JKLightspeedManager manager].remoteNotificationInfo[@"topic_id"]) {
            /* reset notification info */
            [JKLightspeedManager manager].remoteNotificationInfo = [[NSMutableDictionary alloc]initWithCapacity:0];
...
+ (void)saveChatMessageToLocal:(NSArray *)messages withReceiverId:(NSString *)receiverId
{
    for (AnIMMessage *message in messages)
    {
        ...
        if ([dic[@"clientId"] isEqualToString:[JKLightspeedManager manager].clientId]) {
...
+ (void)updatedTopicSessionWithUsers:(NSSet *)users topicId:(NSString *)topicId topicName:(NSString *)topicName topicOwner:(NSString *)topicOwner
{
    ...
    for (NSString *clientId in userClientIds){
        ...
        if (![hxUser.clientId isEqualToString:[JKLightspeedManager manager].clientId]) {
